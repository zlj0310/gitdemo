...
function webpackServer(args, buildConfig, cb) {
  ...
  try {
    ...
    serverConfig = (0, _getUserConfig2.default)(args, { pluginConfig }).devServer;
  } catch (e) {
    return cb(e);
  }
  ...
  getServerPort(args, function (err, port) {
    ...
    if (!('status' in buildConfig.plugins)) {
      buildConfig.plugins.status = {
        ...
        successMessage: `The app is running at http${serverConfig.https ? 's' : ''}://${args.host || 'localhost'}:${port}/`
      };
    }
    ...
    (0, _devServer2.default)(webpackConfig, serverConfig, cb);
  });
}
