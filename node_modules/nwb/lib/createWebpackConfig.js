...
function createUglifyConfig(userPluginConfig) {
  if (userPluginConfig.debug) {
    return (0, _webpackMerge2.default)(_extends({}, DEFAULT_UGLIFY_CONFIG, { beautify: true, mangle: false }),
    // Preserve user 'compress' config if present, as it affects what gets
    // removed from the production build.
    typeof userPluginConfig.uglify === 'object' && 'compress' in userPluginConfig.uglify ? { compress: userPluginConfig.uglify.compress } : {});
  }
  return (0, _webpackMerge2.default)(DEFAULT_UGLIFY_CONFIG, typeof userPluginConfig.uglify === 'object' ? userPluginConfig.uglify : {});
}
...
function createPlugins(server) {
  ...
  if (production) {
    ...
    if (userConfig.uglify !== false) {
      plugins.push(new _webpack.optimize.UglifyJsPlugin(createUglifyConfig(userConfig)));
    }
